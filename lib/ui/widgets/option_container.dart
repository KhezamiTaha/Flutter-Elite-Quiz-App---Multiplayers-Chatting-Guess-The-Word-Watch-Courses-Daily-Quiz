...
class _OptionContainerState extends State<OptionContainer>
  ...
  Color _buildOptionBackgroundColor() {
    if (widget.answerMode == AnswerMode.noAnswerCorrectness) {
      return isSubmittedAnswer
          ? Theme.of(context).primaryColor
          : const Color(0xFFFFFFFF); /// taha 0xFFFFF4EC
    }
  ...
  Widget _buildModernOptionButton(double optionWidth) {
    return AnimatedBuilder(
      animation: animationController,
      builder: (_, child) {
        return Transform.scale(
          scale: animation.drive(Tween<double>(begin: 1, end: 0.95)).value,
          child: child,
        );
      },
      child: Container(
        ...
        child: Material(
          ...
          child: InkWell(
            onTap: _onTapOptionContainer,
            borderRadius: BorderRadius.circular(12),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: const Color(0xFFFFD7B9), width: 1), /// taha 0xFFFFD2AD
              ),
              alignment: Alignment.center,
              child: Text(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: widget.showAudiencePoll
          ? Row(
        children: [
          _buildModernOptionButton(widget.constraints.maxWidth * .8),
          const SizedBox(width: 10),
          Text(
            '${widget.audiencePollPercentage ?? 0}%',
            style: GoogleFonts.nunito(
              fontSize: 16,
              fontWeight: FontWeights.bold,
              color: Theme.of(context).colorScheme.onTertiary,
          ...
          : _buildModernOptionButton(widget.constraints.maxWidth),
    );
  }
}
